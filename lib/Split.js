...
Split.prototype.encode = function encode(value) {
  ...
  const encoded = new Uint8Array(this.branchIndexEncodingLength + valueEncoded.length)
  const padding =  this.branchIndexEncodingLength - branchIndexEncoding.length
...
Split.prototype.decodeWithRemainder = function decodeWithRemainder(encoded) {
  ...
  const branch = this.branches[branchIndex]
  const decodedWithRemainder = this.branches[branchIndex].decodeWithRemainder(encoded.slice(this.branchIndexEncodingLength))
  return {
    value: {
      branch: branchName,
